...
class CSP(search.Problem):
    ...
    # é importante??????? Porque fazer isto no fim???        
...
def nosso_mrv(assignment, csp):
    "Minimum-remaining-values heuristic."
    return argmin_random_tie(
        [v for v in csp.variables if v not in assignment],
        key=lambda var: nosso_num_legal_values(csp, var, assignment))
...
def nosso_num_legal_values(csp, var, assignment):
    if csp.curr_domains:
        return len(csp.curr_domains[var])
    else:
